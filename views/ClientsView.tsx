...
const ClientsView: React.FC<ClientsViewProps> = ({ parents, setParents, children, setChildren, workshops, registrations, setRegistrations, payments, setPayments, locations }) => {
  // Parent state
  ...
  const handleSaveParent = (e: React.FormEvent) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      if (editingClient) {
        setParents(prev => prev.map(p => p.id === editingClient.id ? { ...editingClient, ...dataToSave } as Parent : p));
  ...
  const handleConfirmDeleteClient = () => {
    if (deletingClientId) {
      ...
      const childrenToDeleteIds = childrenToDelete.map(c => c.id);
      
      setParents(prev => prev.filter(p => p.id !== deletingClientId));
      setChildren(prev => prev.filter(c => c.parentId !== deletingClientId));
      setRegistrations(prev => prev.filter(r => !childrenToDeleteIds.includes(r.childId)));
      setPayments(prev => prev.filter(p => p.parentId !== deletingClientId));
      
      alert(`Cliente e tutti i dati associati sono stati eliminati.`);
  ...
  const handleSaveChild = (e: React.FormEvent) => {
    ...
    if (Object.keys(newErrors).length === 0) {
        ...
        if (childModalState?.mode === 'edit') {
            const updatedChild = { ...childModalState.child, ...newChildData };
  ...
  const handleConfirmChildDelete = () => {
    if (deletingChildId) {
      setChildren(prev => prev.filter(c => c.id !== deletingChildId));
      setRegistrations(prev => prev.filter(r => r.childId === deletingChildId));
  ...
  const handleSaveRegistration = (e: React.FormEvent) => {
   ...
   const handleConfirmRegistrationDelete = () => {
    if (deletingRegistrationId) {
      ...
      // Note: We don't delete the payment as it might cover other children. This is a business logic decision.
