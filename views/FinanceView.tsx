...
interface FinanceViewProps {
    ...
    setCosts: React.Dispatch<React.SetStateAction<OperationalCost[]>>;
    quotes: Quote[];
...
const FinanceView: React.FC<FinanceViewProps> = ({
    companyProfile,
    payments, setPayments, costs, setCosts, quotes, setQuotes, invoices, setInvoices,
    ...
    const handleConfirmDelete = () => {
        if(deletingItemId) {
            switch(activeTab) {
                case 'payments': setPayments(prev => prev.filter(i => i.id !== deletingItemId)); break;
                case 'costs': setCosts(prev => prev.filter(i => i.id !== deletingItemId)); break;
                case 'quotes': setQuotes(prev => prev.filter(i => i.id !== deletingItemId)); break;
                case 'invoices': setInvoices(prev => prev.filter(i => i.id !== deletingItemId)); break;
            }
    ...
    const handleSave = (e: React.FormEvent) => {
        ...
        if(validate()){
            ...
            if(editingItem) {
                const updatedItem = { ...editingItem, ...dataToSave };
                switch(activeTab) {
                    case 'payments': setPayments(prev => prev.map(i => i.id === updatedItem.id ? updatedItem as Payment : i)); break;
                    case 'costs': setCosts(prev => prev.map(i => i.id === updatedItem.id ? updatedItem as OperationalCost : i)); break;
                    case 'quotes': setQuotes(prev => prev.map(i => i.id === updatedItem.id ? updatedItem as Quote : i)); break;
                    case 'invoices': setInvoices(prev => prev.map(i => i.id === updatedItem.id ? updatedItem as Invoice : i)); break;
                }
                 alert(`Elemento "${activeTab.slice(0, -1)}" aggiornato!`);
            } else {
                switch(activeTab) {
                    case 'payments': setPayments(prev => [...prev, baseItem as Payment]); break;
                    case 'costs': setCosts(prev => [...prev, baseItem as OperationalCost]); break;
                    case 'quotes': setQuotes(prev => [...prev, baseItem as Quote]); break;
                    case 'invoices': setInvoices(prev => [...prev, baseItem as Invoice]); break;
                }
