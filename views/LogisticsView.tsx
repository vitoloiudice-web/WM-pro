...
const LogisticsView: React.FC<LogisticsViewProps> = ({ suppliers, setSuppliers, locations, setLocations }) => {
    ...
    const handleConfirmDelete = () => {
        if(deletingItemId) {
            if (activeTab === 'suppliers') {
                setSuppliers(prev => prev.filter(s => s.id !== deletingItemId));
                // Also remove supplierId from locations that might reference it
                setLocations(prev => prev.map(loc => loc.supplierId === deletingItemId ? { ...loc, supplierId: undefined } : loc));
            } else {
                setLocations(prev => prev.filter(l => l.id !== deletingItemId));
            }
    ...
    const handleSave = (e: React.FormEvent) => {
        ...
        if (validate()) {
            ...
            if (editingItem) {
                const updatedItem = { ...editingItem, ...preparedData };
                if (activeTab === 'suppliers') {
                    setSuppliers(prev => prev.map(s => s.id === updatedItem.id ? updatedItem as Supplier : s));
                } else {
                    setLocations(prev => prev.map(l => l.id === updatedItem.id ? updatedItem as Location : l));
                }
                alert(`${activeTab === 'suppliers' ? 'Fornitore' : 'Luogo'} aggiornato!`);
            } else {
                 const newItem = { id: `item_${Date.now()}`, ...preparedData };
                 if (activeTab === 'suppliers') {
                    setSuppliers(prev => [...prev, newItem as Supplier]);
                 } else {
                    setLocations(prev => [...prev, newItem as Location]);
                 }
